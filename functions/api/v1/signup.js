...
export async function onRequest(context) {
    ...
    try{
        const user = create_user(await context.request.formData());
        const exist = await user_exist(user,context.env);
        ...
        if (exist){
            ...
            await add_user_to_db(user,context.env);
...
async function add_user_to_db(user, env) {

    const q = 'INSERT INTO users (user_id, user_name, user_type, email_address, password, created_at, preferences, score) VALUES (?1,?2,?3,?4,?5,?6,?7,?8)';
    const exec = await env.DB.prepare(q).bind(user.user_id, user.user_name, user.user_type, user.email_address, user.password, user.created_at, user.preferences ,user.score).run();
...
export async function user_exist(user,env) {
